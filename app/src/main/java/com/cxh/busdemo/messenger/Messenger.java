...
public class Messenger {
    ...
    private static <T> void sendToList(T message, Collection<WeakActionAndToken> list, Type messageTargetType, Object token) throws Exception {
        if (list != null) {
    ...
    private static <T> void unregisterFromLists(Object recipient, Consumer<T> action, HashMap<Type, List<WeakActionAndToken>> lists, Class<T> tClass) {
        Type messageType = tClass;
    ...
    private static void unregisterFromLists(Object recipient, Action action, HashMap<Type, List<WeakActionAndToken>> lists) {
        Type messageType = NotMsgType.class;
    ...
    private static <T> void unregisterFromLists(Object recipient, Object token, Consumer<T> action, HashMap<Type, List<WeakActionAndToken>> lists, Class<T> tClass) {
        Type messageType = tClass;
    ...
    private static void unregisterFromLists(Object recipient, Object token, Action action, HashMap<Type, List<WeakActionAndToken>> lists) {
        Type messageType = NotMsgType.class;
...
//            var listClone = recipientsOfSubclassesAction.Keys.Take(_recipientsOfSubclassesAction.Count()).ToList();
    ...
    private static void sendToList(Collection<WeakActionAndToken> list, Type messageTargetType, Object token) throws Exception {
        if (list != null) {
            // Clone to protect from people registering in a "receive message" method
            // Bug correction Messaging BL0004.007
            ArrayList<WeakActionAndToken> listClone = new ArrayList<>();
